...
#define RESET_VECTOR_CLEAN_IN_C_MAGIC  0xDCBAABCD
#define MSM_RESET_VECTOR_BASE (MSM_POWER_COLLAPSE_STATUS_BASE - 8)
static void clean_reset_vector_debug_info(unsigned cpu)
{
	unsigned *reset_vector = (unsigned *)MSM_RESET_VECTOR_BASE;
	reset_vector[cpu] = RESET_VECTOR_CLEAN_IN_C_MAGIC;
	mb();
}

unsigned int *tz_entry_0 =(unsigned int *) (MSM_SCL_HTC_PRIV_BASE - 24);
unsigned int *tz_exit_0 = (unsigned int *)(MSM_SCL_HTC_PRIV_BASE - 32);
unsigned int *msm_kernel_exit_0 = (unsigned int *)(MSM_SCL_HTC_PRIV_BASE - 40);
unsigned int *msm_kernel_exit_1 = (unsigned int *)(MSM_SCL_HTC_PRIV_BASE - 48);
unsigned int *tz_entry_1 =(unsigned int *) (MSM_SCL_HTC_PRIV_BASE - 56);
unsigned int *tz_exit_1 = (unsigned int *)(MSM_SCL_HTC_PRIV_BASE - 64);
unsigned int *tz_progress = (unsigned int *)(MSM_SCL_HTC_PRIV_BASE - 72);
unsigned int *tz_kernel_addr = (unsigned int *)(MSM_SCL_HTC_PRIV_BASE - 80);
unsigned int *tz_error = (unsigned int *)(MSM_SCL_HTC_PRIV_BASE - 88);
unsigned int *cpu1_pen_rejection = (unsigned int *)(MSM_SCL_HTC_PRIV_BASE - 96);
unsigned int *cpu1_wakeup_reason = (unsigned int *)(MSM_SCL_HTC_PRIV_BASE - 104);
	...
	struct msm_pm_device *dev, bool from_idle, bool notify_rpm)
{
	...
	clean_reset_vector_debug_info(dev->cpu);
...
int msm_pm_idle_enter(enum msm_pm_sleep_mode sleep_mode)
{
	...
	case MSM_PM_SLEEP_MODE_POWER_COLLAPSE: {
		...
		/* check if idle time is greater than 60s. */
		writel(sleep_delay, MSM_IDLE_SLEEP_DELAY_BASE);
		if (sleep_delay > (SCLK_HZ * 60))
			pr_err("%s: sleep_delay is too long, %d\n", __func__, sleep_delay);
		...
		/* check if timer long jump during idle */
		t1 = jiffies;
		msm_timer_exit_idle((int) timer_halted);
		t2 = jiffies;
		if (t2 - t1 > HZ * 60)
			pr_err("%s: timer long jump, t1 %ld, t2 %ld\n", __func__, t1, t2);
#ifdef CONFIG_MSM_IDLE_STATS
