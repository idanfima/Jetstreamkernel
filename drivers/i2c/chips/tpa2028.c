/* driver/i2c/chip/tap2028.c
 *
 * TI tpa2028 Speaker Amp
 *
...
static struct tpa2028_platform_data *pdata;
static char *config_data;
static int tpa2028_mode_cnt;
struct mutex spk_amp_lock_2028;
static int tpa2028_opened;
static int last_spkamp_state;
static int mfg_mode_2028;
...
static char AMP_0FF[] = {0xE2, 0x05, 0x01, 0x00, 0x30, 0x9A, 0xC0};
static char MFG_LOOPBACK_ON[]	= {0xC2, 0x05, 0x01, 0x00, 0x06, 0x9A, 0xC0};
static char MFG_LOOPBACK_OFF[]	= {0x82, 0x05, 0x01, 0x00, 0x06, 0x9A, 0xC0};

struct pm8058_gpio tpa2028pwr = {
...
static int tpa2028_write_reg(u8 reg, u8 val)
{
	int err;
...
static int tpa2028_i2c_write(char *txData, int length)
{
	int i, retry, pass = 0;
	char buf[2];
	struct i2c_msg msg[] = {
	...
	for (i = 0; i < length; i++) {
		if (i == 2)  /* According to tpa2028 Spec */
			mdelay(1);
		buf[0] = (i+1);
		buf[1] = txData[i];
/* #if DEBUG */
...
static int tpa2028_i2c_write_for_read(char *txData, int length)
{
	int i, retry, pass = 0;
	char buf[2];
	struct i2c_msg msg[] = {
...
static int tpa2028_open(struct inode *inode, struct file *file)
{
	...
	mutex_unlock(&spk_amp_lock_2028);
	return rc;
}

static int tpa2028_release(struct inode *inode, struct file *file)
{
	mutex_lock(&spk_amp_lock_2028);
	tpa2028_opened = 0;
	mutex_unlock(&spk_amp_lock_2028);
...
void set_amp_2028(int on, char *i2c_command)
{
	pr_info("%s: %d\n", __func__, on);
	mutex_lock(&spk_amp_lock_2028);
	if (on && !last_spkamp_state) {
		if (tpa2028_i2c_write(i2c_command, AMP_ON_CMD_LEN) == 0) {
			last_spkamp_state = 1;
			pr_info("%s: ON reg1=%x, reg2=%x\n",
				__func__, i2c_command[1], i2c_command[2]);
		}
	} else if (!on && last_spkamp_state) {
		if (tpa2028_i2c_write(AMP_0FF, sizeof(AMP_0FF)) == 0) {
			last_spkamp_state = 0;
			pr_info("%s: OFF\n", __func__);
		}
	}
...
{
	switch (mfg_mode_2028) {
	case 1:
		pr_info("%s: MFG_LOOPBACK_ON\n", __func__);
		set_amp_2028(on, MFG_LOOPBACK_ON);
		break;
	case 2:
		pr_info("%s: MFG_LOOPBACK_OFF\n", __func__);
		set_amp_2028(on, MFG_LOOPBACK_OFF);
		break;
	case 0:
	default:
		set_amp_2028(on, SPK_AMP_ON);
		break;
	}
}
...
{
	set_amp_2028(on, HEADSET_AMP_ON);
}

void set_speaker_headset_amp_2028(int on)
{
	set_amp_2028(on, RING_AMP_ON);
}

void set_handset_amp_2028(int on)
{
	set_amp_2028(on, HANDSET_AMP_ON);
}

int update_amp_parameter_2028(int mode)
{
	if (mode > tpa2028_mode_cnt)
		return EINVAL;
...
static int
tpa2028_ioctl(struct inode *inode, struct file *file, unsigned int cmd,
	   unsigned long arg)
{
	...
	struct tpa2028_config_data cfg;

	switch (cmd) {
	case TPA2028_WRITE_REG:
		pr_info("%s: TPA2028_WRITE_REG\n", __func__);
		mutex_lock(&spk_amp_lock_2028);
		if (copy_from_user(reg_value, argp, sizeof(reg_value)))
			goto err1;
		pr_info("%s: reg_value[0]=%2x, reg_value[1]=%2x\n", __func__,  \
				reg_value[0], reg_value[1]);
		rc = tpa2028_write_reg(reg_value[0], reg_value[1]);
...
err1:
	...
	case TPA2028_READ_CONFIG:
		mutex_lock(&spk_amp_lock_2028);
		rc = tpa2028_i2c_write_for_read(reg_idx, sizeof(reg_idx));
		if (rc < 0)
			goto err2;
...
err2:
	...
	case TPA2028_SET_MODE:
		...
		if (modeid > tpa2028_mode_cnt || modeid <= 0) {
			pr_err("unsupported tpa2028 mode %d\n", modeid);
			return -EINVAL;
		}
		rc = update_amp_parameter_2028(modeid);
		pr_info("set tpa2028 mode to %d\n", modeid);
		break;
	case TPA2028_SET_PARAM:
		...
		tpa2028_mode_cnt = cfg.mode_num;
		pr_info("%s: update tpa2028 i2c commands #%d success.\n",
				__func__, cfg.data_len);
		/* update default paramater from csv*/
		update_amp_parameter_2028(TPA2028_MODE_PLAYBACK_SPKR);
		update_amp_parameter_2028(TPA2028_MODE_PLAYBACK_HEADSET);
		update_amp_parameter_2028(TPA2028_MODE_RING);
		update_amp_parameter_2028(TPA2028_MODE_HANDSET);
		rc = 0;
		break;
	case TPA2028_SET_MFG_LOOPBACK_ON:
		mutex_lock(&spk_amp_lock_2028);
		/* LOOPBACK mode turn ON */
		mfg_mode_2028 = 1;
		pr_info("%s: TPA2028_SET_MFG_LOOPBACK_ON\n", __func__);
		mutex_unlock(&spk_amp_lock_2028);
		break;
	case TPA2028_SET_MFG_LOOPBACK_OFF:
		mutex_lock(&spk_amp_lock_2028);
		/* LOOPBACK mode mute */
		mfg_mode_2028 = 2;
		pr_info("%s: TPA2028_SET_MFG_LOOPBACK_OFF\n", __func__);
		mutex_unlock(&spk_amp_lock_2028);
		break;
	case TPA2028_SET_MFG_LOOPBACK_STOP:
		mutex_lock(&spk_amp_lock_2028);
		/* LOOPBACK mode END */
		mfg_mode_2028 = 0;
		pr_info("%s: TPA2028_SET_MFG_LOOPBACK_STOP\n", __func__);
		mutex_unlock(&spk_amp_lock_2028);
		break;
	case TPA2028_RESET:
		tpa2028pwr.output_value = 0;
		rc = pm8058_gpio_config(pdata->gpio_tpa2028_spk_en, \
				&tpa2028pwr);
		pr_info("%s: TPA2028_RESET!\n", __func__);
		mdelay(100);
		pr_info("%s: after TPA2028_RESET, turn gpio on\n", __func__);
		tpa2028pwr.output_value = 1;
		rc = pm8058_gpio_config(pdata->gpio_tpa2028_spk_en, \
				&tpa2028pwr);
		break;
	default:
...
static struct file_operations tpa2028_fops = {
	.owner = THIS_MODULE,
	.open = tpa2028_open,
	.release = tpa2028_release,
	.ioctl = tpa2028_ioctl,
};

static struct miscdevice tpa2028_device = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = "tpa2028",
	.fops = &tpa2028_fops,
};

int tpa2028_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	int ret = 0;
	...
	ret = misc_register(&tpa2028_device);
	if (ret) {
		pr_info("%s: tpa2028_device register failed\n", __func__);
		goto err_free_gpio_all;
	}
	...
	tpa2028pwr.output_value = 1;
	ret = pm8058_gpio_config(pdata->gpio_tpa2028_spk_en,
							&tpa2028pwr);
	last_spkamp_state = 1;
	msleep(1);
...
{
	struct tpa2028_platform_data *p2028data = i2c_get_clientdata(client);
	kfree(p2028data);
...
{
	pr_info("%s\n", __func__);
	mutex_init(&spk_amp_lock_2028);
	mfg_mode_2028 = 0;
	return i2c_add_driver(&tpa2028_driver);
}
...
MODULE_DESCRIPTION("tpa2028 Speaker Amp driver");
MODULE_LICENSE("GPL");
